...
TRIGGER_REGEX = {
    ...
    "avpc": (
        ...
        r"(?:prostate|prostatic|pelvic|nodal|lymph\s*node|lymphadenopathy)"
        r".{0,80}\b(?:[5-9]|[1-9]\d+)(?:\.\d+)?\s*cm\b|"
        r"\b(?:[5-9]|[1-9]\d+)(?:\.\d+)?\s*cm\b.{0,80}"
        r"(?:prostate|prostatic|pelvic|nodal|lymph\s*node|lymphadenopathy)|"
        r"low\s+psa|disproportionately\s+low\s+psa|psa\s+discordant|"
        r"psa\s*(?:level\s*)?(?:of|=|:)?\s*(?:10(?:\.0+)?|[0-9](?:\.\d+)?)"
        r"\s*(?:ng\s*/?\s*m[lL])?|"
        r"(?:high[- ]volume|extensive|diffuse|innumerable)\s+(?:bone|osseous)\s+met(?:astases|astatic)?|"
        r"(?:20|[2-9]\d|[1-9]\d{2,})\s+(?:bone|osseous)\s+(?:metastases|lesions)|"
        r"bombesin|grp|cea|ldh|hypercalc(?:emia|aemia)|"
        r"castration[- ]resistant|androgen[- ]independent|rapidly?\s+progress(?:ion|ive)|"
        r"refractory\s+to\s+adt|despite\s+adt|androgen\s+deprivation(?:\s+therapy)?|"
        r"(?:started|initiated|begin|began).{0,60}(?:adt|lupron|leuprolide|degarelix|relugolix)"
        r")\b"
    ),
    "biomarker": (
        r"\b(?:"
