"""Adjudicate a single strict NEPC diagnosis label + date per patient.

Stage 1 (`preprocessing/cli/collect_nepc_dx_notes.py`) writes trigger-bearing
evidence chunks. This stage maps every chunk into candidate diagnostic
statements, then adjudicates all candidates for a patient into one yes/no label
with an earliest diagnosis date. Chunk maps and patient adjudications are
durable resume state.

Precision-over-recall differences from tasks/longitudinal_NEPC:
  ...
  - Every grounded quote must clear the deterministic gate in
    tasks/nepc_diagnosis/veto.py, which the model cannot override.
...
from tasks.nepc_diagnosis.prompts import (  # noqa: E402
    NEPC_DX_MAP_PROMPT,
    NEPC_DX_SYNTHESIS_PROMPT,
    PROMPT_SCHEMA_VERSION,
)
from tasks.nepc_diagnosis.veto import VETO_VERSION, screen_quote  # noqa: E402

DEFAULT_OUTPUT_DIR = Path(DEFAULT_DATA_PATH) / "LLM_nepc_diagnosis"

VALID_EVIDENCE_TYPES = {"stated_diagnosis", "histologic_transformation"}
VALID_ASSERTION_TYPES = {"established", "reported_history"}
VALID_MODALITIES = {"pathology", "clinical"}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Adjudicate one strict NEPC diagnosis label and date per patient."
    )
...
def extraction_run_config(scan_config, provider_name, model):
    ...
    payload = {
        ...
        "model": model,
        "map_prompt": NEPC_DX_MAP_PROMPT,
        "synthesis_prompt": NEPC_DX_SYNTHESIS_PROMPT,
...
def check_resume_config(
    ...
    if meta.get("evidence_schema_version") != NEPC_DX_EVIDENCE_SCHEMA_VERSION:
        raise ValueError(
            "NEPC diagnosis evidence predates the current trigger/cohort "
            "contract. Regenerate evidence with --overwrite."
        )
...
def _validate_grounded_quote(item, notes):
    ...
    Returns (quote, source_note_date, error). Unlike the longitudinal pipeline's
    _find_support_note, a quote that occurs only in a differently-dated note is
    rejected outright rather than having its provenance silently corrected.
    """
...
def _validate_common_fields(item, notes):
    """Shared enum/grounding/date/veto gauntlet for map and synthesis findings."""
    ...
    return {
        ...
        "stated_diagnosis_date": stated_iso,
    }, None
...
def validate_map_result(result, chunk):
    ...
    for item in candidates:
        ...
        key = (
            ...
            normalized["stated_diagnosis_date"],
        )
        if key in seen:
            continue
        seen.add(key)
...
def validate_synthesis_result(result, notes, candidate_index):
    ...
    if not isinstance(result, dict):
        return None, f"non_dict_response:{type(result).__name__}"
    has_dx = result.get("has_nepc_diagnosis")
    if not isinstance(has_dx, bool):
        # No truthiness coercion of "yes"/1: an ambiguous label is unusable.
        return None, f"invalid_has_nepc_diagnosis:{has_dx}"
    ...
    if not has_dx:
        evidence_fields = (
            "evidence_type", "assertion_type", "diagnosis_date",
    ...
    # The synthesis prompt names the quote field differently from the map stage;
    # normalize before running the shared validator.
    item = dict(result)
    item["quote"] = result.get("supporting_quote")
    item["stated_diagnosis_date"] = result.get("diagnosis_date")
...
def _extract_chunk(provider, client, model, max_retries, mrn, chunk_index, chunk):
    ...
    messages = [
        {"role": "system", "content": NEPC_DX_MAP_PROMPT + CLINICAL_SAFETY_CONTEXT},
...
def synthesize_patient(provider, client, model, max_retries, mrn, chunk_outputs, chunks):
    ...
    messages = [
        {"role": "system", "content": NEPC_DX_SYNTHESIS_PROMPT + CLINICAL_SAFETY_CONTEXT},
        {"role": "user", "content": json.dumps(payload, ensure_ascii=False)},
    ]
    result, error = _call_json(provider, client, model, max_retries, messages)
    if error:
                ...
                rows.append(
                    {
                        ...
                        "stated_diagnosis_date": candidate.get("stated_diagnosis_date"),
...
def _earliest_supporting_note_date(chunk_outputs, evidence_type):
    """Earliest source note date among candidates matching the adjudicated type.

    Used only when the adjudicated finding states no date. The supporting quote
    still comes from the adjudicated candidate; this only makes the fallback
    date the earliest documentation of the same finding rather than whichever
    note the model happened to cite.
    """
    dates = [
        candidate.get("source_note_date")
        for result in chunk_outputs.values()
        ...
        and candidate.get("source_note_date")
    ]
...
def build_labels(
    ...
    for mrn in sorted(completed):
        ...
        if not result.get("has_nepc_diagnosis") or not isinstance(finding, dict):
            rows.append(
                {
                    "DFCI_MRN": mrn,
                    "has_nepc_diagnosis": False,
        ...
        stated = finding.get("stated_diagnosis_date")
        ...
        if date_source == "note_date":
            ...
            # "note_date" so the fallback remains filterable downstream.
            earliest = _earliest_supporting_note_date(
                outputs, finding.get("evidence_type")
            )
...
def run(args):
    ...
    evidence_path = args.evidence_path or (args.output_dir / "nepc_dx_evidence.parquet")
    meta_path = getattr(args, "evidence_meta_path", None) or _meta_path_for_evidence(
        evidence_path
    )
    candidates_path = args.output_dir / "nepc_dx_candidates_raw.parquet"
    chunk_log_path = args.output_dir / "nepc_dx_processed_chunks.parquet"
    processed_path = args.output_dir / "nepc_dx_processed_patients.parquet"
    labels_path = args.output_dir / "nepc_dx_labels.parquet"
    rejected_path = args.output_dir / "nepc_dx_rejected_findings.parquet"

    if not evidence_path.exists():
        raise FileNotFoundError(
            f"Evidence table not found: {evidence_path}\n"
            "Run preprocessing/cli/collect_nepc_dx_notes.py first."
        )
    if args.overwrite:
        for path in (
            candidates_path,
    ...
    if getattr(args, "rebuild_labels_only", False):
        ...
        print(f"Wrote NEPC diagnosis labels ({label_count} rows): {labels_path}")
        return
